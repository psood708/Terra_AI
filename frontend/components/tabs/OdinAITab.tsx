...
const PROMPT_CHIPS = [
  { label: 'Why is my recovery at this score?', query: 'Why is my recovery score at this level today?', color: 'text-emerald-400' },
  { label: 'What workout should I do?', query: 'What is my recommended adaptive workout today?', color: 'text-cyan-400' },
  { label: 'Analyze my glucose & CGM', query: 'Analyze my blood glucose excursions and post-meal response.', color: 'text-amber-400' },
  { label: 'Tracking for Target 120', query: 'How am I tracking towards extreme longevity and Target 120?', color: 'text-teal-400' },
];
      ...
      <div id="tour-odin" className="lg:col-span-2 rounded-2xl p-5 flex flex-col" style={{ height: '580px', background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        ...
        <div className="flex items-center justify-between pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <button onClick={onOpenAiModal}
            className="text-[10px] text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 px-2.5 py-1 rounded-full border border-emerald-500/20 flex items-center gap-1.5 transition">
        ...
        <div ref={chatRef} className="flex-1 overflow-y-auto py-4 space-y-4 pr-1 text-xs">
            ...
            <div key={msg.id}>
              {msg.role === 'user' ? (
                ...
                <div className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-full bg-emerald-600/30 text-emerald-400 flex items-center justify-center text-xs shrink-0 mt-0.5">
                    <Dna className="w-3.5 h-3.5" />
                  </div>
                  <div className="rounded-xl rounded-tl-none p-3.5 text-xs space-y-2 max-w-xl shadow" style={{ background: 'rgba(17,24,39,0.8)', border: '1px solid var(--border)' }}>
                    {msg.isLiveAi !== undefined && (
                      <div className="flex items-center mb-1">
                        {msg.isLiveAi ? (
                          ...
                          <button onClick={onOpenAiModal} className="text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded-full font-semibold flex items-center gap-1">
                    ...
                    {msg.llmError && (
                      <div className="text-[10px] text-amber-400 bg-amber-500/10 border border-amber-500/25 rounded-lg p-2 leading-relaxed">
          ...
          {isLoading && (
            <div className="flex items-start gap-3">
              <div className="w-7 h-7 rounded-full bg-emerald-600/30 text-emerald-400 flex items-center justify-center text-xs shrink-0">
                <Dna className="w-3.5 h-3.5 animate-spin" />
              </div>
              <div className="rounded-xl rounded-tl-none p-3 text-xs italic" style={{ background: 'rgba(17,24,39,0.8)', border: '1px solid var(--border)', color: 'var(--text-muted)' }}>
      ...
      <div className="space-y-6">
              ...
              anomalies.anomalies.map((a, i) => (
                <div key={i} className="rounded-lg p-2.5 space-y-1" style={{ border: '1px solid rgba(244,63,94,0.3)', background: 'var(--card-inner)' }}>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-rose-400 uppercase text-[10px]">{a.metric}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-950 text-rose-300">{a.detected_value}</span>
                  </div>
                  <p style={{ color: 'var(--text-sub)' }}>{a.message}</p>
                  <div className="text-[10px] text-emerald-400">💡 {a.actionable_fix}</div>
                </div>
              ))
            )}
          </div>
        ...
        <div className="rounded-2xl p-5 text-xs" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <h4 className="text-xs font-bold flex items-center gap-2 mb-3">
            <BookMarked className="w-3.5 h-3.5 text-emerald-400" /> Scientific Grounding
          </h4>
