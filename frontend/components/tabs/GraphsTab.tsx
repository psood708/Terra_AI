    ...
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold flex items-center gap-2">
                <span className="text-amber-400">📊</span> Ambulatory Glucose Profile (AGP) 24h
              </h3>
              <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Clinical Target: 70–140 mg/dL</p>
            </div>
            {agp && <div className="text-xs font-semibold px-2.5 py-1 rounded bg-amber-500/10 text-amber-400 border border-amber-500/20">TIR: {agp.agp_metrics.time_in_range_pct}%</div>}
          </div>
          ...
          {agp && (
            <div className="grid grid-cols-3 gap-2 mt-4 pt-3 text-center text-xs" style={{ borderTop: '1px solid var(--border)' }}>
              ...
              <div><span className="text-[10px] block" style={{ color: 'var(--text-muted)' }}>Est. A1c (GMI)</span><strong className="text-emerald-400">{agp.agp_metrics.gmi_estimated_a1c}%</strong></div>
        ...
        <div className="rounded-2xl p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold flex items-center gap-2">
                <span className="text-indigo-400">🛀</span> Sleep Architecture & Hypnogram
              </h3>
              <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Deep, REM, Light, and Awake breakdown</p>
            </div>
            {hypnogram && <div className="text-xs font-semibold px-2.5 py-1 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Score: {hypnogram.sleep_score}</div>}
          </div>
          ...
          {hypnogram && (
            <div className="grid grid-cols-4 gap-2 mt-4 pt-3 text-center text-xs" style={{ borderTop: '1px solid var(--border)' }}>
              <div><span className="text-[10px] block" style={{ color: 'var(--text-muted)' }}>Deep</span><strong className="text-indigo-300">{hypnogram.stage_breakdown_minutes.deep}m</strong></div>
              <div><span className="text-[10px] block" style={{ color: 'var(--text-muted)' }}>REM</span><strong className="text-cyan-300">{hypnogram.stage_breakdown_minutes.rem}m</strong></div>
              ...
              <div><span className="text-[10px] block" style={{ color: 'var(--text-muted)' }}>Awake</span><strong className="text-rose-400">{hypnogram.stage_breakdown_minutes.awake}m</strong></div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl p-5" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold flex items-center gap-2">
              <span className="text-teal-400">🔗</span> 14-Day Multi-Stream Longitudinal Coupling
            </h3>
