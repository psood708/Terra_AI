  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-xl px-4 py-3 text-xs transition-colors"
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <span className="text-emerald-400 light:text-emerald-700 font-medium">{persona?.target_goal ?? '—'}</span>
          </div>
        </div>
      ...
      {anomalies && anomalies.anomalies_count > 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-xs text-amber-200 light:text-amber-800 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <span className="text-amber-400 light:text-amber-700 text-sm">⚠</span>
            <span>{anomalies.anomalies_count} Biometric Alert(s): {anomalies.anomalies[0]?.message}</span>
          </div>
          <button onClick={onAskOdin} className="text-amber-300 light:text-amber-800 underline hover:text-[color:var(--text-primary)] font-medium">Ask OdinAI</button>
        </div>
      )}
    </div>
  );
